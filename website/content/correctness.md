...
5. **`--info` predicts what a conversion writes.** The row count is exact and the byte count never reads low, across every fixture crossed with every option combination. Checked by `npm run estimate`: **338 predictions over 45 recordings**, sizes reading 16% high on average, which is the direction a size estimate has to err in.
...
```
248 conversions compared over 45 recordings (568 channel sequences, 22 refused by both).
...
The 259 tests are split across six files by what they exercise:
