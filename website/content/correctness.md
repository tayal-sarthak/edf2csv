...
The 253 tests are split across six files by what they exercise:
...
| `test/convert.test.js` | 78 | Time specifications, option checking, column naming, channel selection, rate grouping, and the contents of the written CSV files |
| `test/cli.test.js` | 112 | The built executable: exit codes, stdout versus stderr, overwrite refusal, unwritable destinations, invocation through a symlink as `npx` does |
