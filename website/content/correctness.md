...
The 293 tests are split across six files by what they exercise:
...
| `test/convert.test.js` | 87 | Time specifications, option checking, column naming, channel selection, rate grouping, and the contents of the written CSV files |
| `test/cli.test.js` | 128 | The built executable: exit codes, stdout versus stderr, overwrite refusal, unwritable destinations, invocation through a symlink as `npx` does |
| `test/docs.test.js` | 25 | That this documentation and the source agree on their lists of codes, flags and exit codes |
