...
The second and ninth are what `npm test` runs; the third through eighth and the eleventh are the batch, fuzz, estimate, round-trip, layout, narrowing and stream commands. The tenth needs a pseudo terminal, which Node cannot allocate, so it borrows python3's `pty` module; where that is unavailable it says it checked nothing and exits 2, which since 0.9.57 is the answer every sweep here gives to "did not run" — a status meaning that must not be the status meaning "agreed". The first needs pyEDFlib, so it is a separate command — the package itself has no dependencies and `npm test` keeps it that way:
...
The checked-in version of this dumper is `test/crossvalidate/dump-doubles.mjs`, which does every channel at once and is what `npm run crossvalidate` runs, so the method printed here and the one that executes are the same code. Since 0.9.70 that is a test rather than a statement: `npm test` runs the recipe above on a fixture and requires the doubles it writes to be the ones the checked-in dumper writes, with only the import rewritten to the build under test. The recipe reaches seven parts of the public API, and a change to any of them used to leave these instructions broken with nothing to say so.
...
The 542 tests are split across six files by what they exercise:
...
| `test/docs.test.js` | 116 | That this documentation and the source agree on their lists of codes, flags and exit codes |
