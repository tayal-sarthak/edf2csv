...
The 507 tests are split across six files by what they exercise:
...
| `test/convert.test.js` | 137 | Time specifications, option checking, column naming, channel selection, rate grouping, and the contents of the written CSV files |
| `test/cli.test.js` | 179 | The built executable: exit codes, stdout versus stderr, overwrite refusal, unwritable destinations, invocation through a symlink as `npx` does. Two cases need a filesystem that folds case or Unicode normalisation and skip where there is none, which is everywhere CI runs |
| `test/docs.test.js` | 111 | That this documentation and the source agree on their lists of codes, flags and exit codes |
