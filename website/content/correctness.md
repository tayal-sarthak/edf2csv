...
5. **`--info` predicts what a conversion writes.** The row count is exact and the byte count never reads low, across every fixture crossed with every option combination. Checked by `npm run estimate`: **332 predictions over 44 recordings**, sizes reading 16% high on average, which is the direction a size estimate has to err in.
...
```
243 conversions compared over 44 recordings (558 channel sequences, 21 refused by both).
...
The 257 tests are split across six files by what they exercise:
...
| `test/convert.test.js` | 80 | Time specifications, option checking, column naming, channel selection, rate grouping, and the contents of the written CSV files |
| `test/cli.test.js` | 114 | The built executable: exit codes, stdout versus stderr, overwrite refusal, unwritable destinations, invocation through a symlink as `npx` does |
| `test/docs.test.js` | 16 | That this documentation and the source agree on their lists of codes, flags and exit codes |
