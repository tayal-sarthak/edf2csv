...
5. **`--info` predicts what a conversion writes.** The row count is exact and the byte count never reads low, across every fixture crossed with every option combination. Checked by `npm run estimate`: **362 predictions over 48 recordings**, sizes reading 16% high on average, which is the direction a size estimate has to err in.
...
The 273 tests are split across six files by what they exercise:
...
| `test/convert.test.js` | 84 | Time specifications, option checking, column naming, channel selection, rate grouping, and the contents of the written CSV files |
...
| `test/docs.test.js` | 21 | That this documentation and the source agree on their lists of codes, flags and exit codes |
