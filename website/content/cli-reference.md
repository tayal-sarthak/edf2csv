...
- `OUTPUT` names the file the channel would land in, `(not selected)` when `--channels` excludes it, `(no samples)` when the channel declares zero samples per data record and so has nothing to put in a file, or `(no signal data)` when the run writes no signal table at all, which is what `--annotations-only` asks for. The three are distinct on purpose: a channel named on `--channels` and carrying no samples reads `(no samples)`, because it was chosen and the file gives it nothing, and the `NO_SAMPLES` warning below the table says so; every channel of an `--annotations-only` run reads `(no signal data)`, because nothing about that run turned on which channels were named.
