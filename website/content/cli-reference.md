...
- `OUTPUT` names the file the channel would land in, `(not selected)` when `--channels` excludes it, or `(no samples)` when the channel declares zero samples per data record and so has nothing to put in a file. The third is not the second: a channel named on `--channels` and carrying no samples reads `(no samples)`, because it was chosen and the file gives it nothing, and the `NO_SAMPLES` warning below the table says so.
