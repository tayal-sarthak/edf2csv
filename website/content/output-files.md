...
sleep-study_csv/
  ...
  metadata.json    what was converted, from what, and what was odd about it
```

Four rules govern which of these appear:
...
All three CSV files use the same dialect, and it is the boring one:
...
The header row is not a comment and is not preceded by any preamble. Row one is column names, row
two is data.
...
to end. A recording that pauses for eight seconds after two seconds of data produces exactly this:
...
The gap stays a gap. Rows are written in file order, so if a file's records are stored out of
chronological order the column will not increase monotonically, and a `DISCONTINUOUS` warning says
so. If a record's timekeeping annotation is missing or unreadable, that record is timed as if it
were contiguous and an `ANNOTATION_DECODE_FAILED` warning names the affected records.
...
256 Hz is the case that matters most in practice. Written with three decimals, sample 1 of a 256 Hz
channel would be `0.004`, and dividing that back by the sample period would not return 1. Written
with eight, it is `0.00390625`, which is the exact value, and `time_s * 256` is an integer for every
row in the file.

Two rates fall outside this. A rate whose reciprocal does not terminate at all, such as 3 Hz, gets
enough places to keep consecutive samples distinct and no more. A rate needing more than nine
places, such as 1024 Hz, is capped and the times are rounded. In both cases the column is marked
"rounded" above: the times are accurate to within a fraction of a sample period, but multiplying
them by the rate will not land on exact integers.

`--decimals` does not affect this column. It sets the precision of the signal values only.

### Column names

A channel's column is its EDF label, copied verbatim. `EEG Fpz-Cz` stays `EEG Fpz-Cz`, spaces,
hyphens, case and all. Nothing is slugified, lowercased or stripped, because the label is how you
recognise the channel and because rewriting it would break the correspondence with the recording's
own documentation.
...
- **Duplicated label.** When two or more channels share a label, every one of them gets a
  ...
  is the only thing that reliably tells the channels apart. A `DUPLICATE_LABEL` warning is raised.
...
places put rounding error well below the resolution the hardware actually recorded, so no two
distinct digital codes ever round to the same text, without padding the file with digits that carry
no information.
...
The upper clamp is 15 rather than something tidier because a channel calibrated in volts rather
than microvolts has a step near 1e-7, and a magnetometer channel smaller still. A lower cap would
round genuinely different samples to the same text.
...
- A value that scales to a very small negative number is written as `0.000`, not `-0.000`. Negative
  zero is not a distinct measurement and printing it as one is misleading.

Pass `--decimals <n>` to override the derived precision and use the same number of places on every
channel. That is useful for diffing two conversions or for shrinking a file, but it can round
distinct samples together, so it is not the default.
...
correctly rounded result, and it is bit-for-bit identical to pyEDFlib and EDFbrowser, which share
the same arithmetic.

## One file per sampling rate

Recordings often mix rates. A sleep study may hold EEG at 256 Hz, ECG at 128 Hz and rectal
temperature at 1 Hz. These cannot share one table without inventing values for the slow channels,
so `edf2csv` does not try. Each distinct rate gets its own file:
...
is a sample that was actually recorded. A `MIXED_SAMPLING_RATES` warning tells you this happened.
...
When every converted channel shares one rate, there is one group and the file is called
...
To join rates you have to decide what to do about the mismatch, which is a modelling decision and
therefore yours. Merging on `time_s` with a nearest or backward-fill strategy is one answer; the
point is that you make it explicitly, with the original sample times in front of you.

## channels.csv

One row per signal channel in the recording, whether or not it was converted. The EDF+ annotation
channel is not a signal and is not listed.
...
| `label` | The label exactly as stored in the EDF header, with no disambiguating suffix. Where two rows share a `label` they will differ in `column`. |
...
| `sampling_rate_hz` | `samples_per_record / record_duration_seconds`. This is what decides which output file the channel lands in. |
...
| `output_file` | Name of the CSV holding this channel's samples, or empty when the channel was not converted. |
| `converted` | `yes` or `no`. |

The four calibration columns are written as they appear in the header, so `physical_min` above
`physical_max` (an inverted channel) survives into the file rather than being quietly corrected. The
...
`converted` is `no` in three situations: you used `--channels` and did not ask for this one, you
used `--annotations-only` so nothing was converted, or the channel declares zero samples per record
and therefore holds no data at all. In every case the row is still present, so `channels.csv` is a
complete description of the recording and not only of what you exported.
...
An absent duration is written as an empty field, never as `0`. This distinction is real: EDF+ lets
an annotation mark an instant with no extent, and writing that as a zero-second event would be a
claim the file does not make. In pandas the column reads as `NaN` with no extra arguments; treat
...
Two things do not appear as rows. The timekeeping annotation that starts each data record carries
the record's position in time and no text, so it is used for timing and not exported as an event.
And annotations whose onset falls outside a requested `--start` / `--end` window are excluded, on
the same half-open rule as the signal rows. Note that the whole annotation channel is read even
when a window was requested, because an event inside the window may be stored in a record outside
it.

If an annotation is malformed, it is skipped rather than aborting the conversion, and an
`ANNOTATION_DECODE_FAILED` warning reports how many were lost.

## metadata.json

A record of what was converted, from what, when, and what was odd about it. This is the file that
makes a conversion reproducible six months later.
...
hash it. With a hash recorded, anyone holding the original can prove the CSVs came from that exact
file, and you can detect a re-export or a partial copy that kept the same size and name. It costs
one extra read of the input and nothing else, so it is worth having on anything you intend to
publish or archive.
...
- `data_records` is how many complete data records the file actually contains, derived from its
  size. `data_records_declared` is what the header claims, and is `-1` when the header does not say,
...
- `duration_seconds` is `data_records * record_duration_seconds`, so for a discontinuous recording
  it is the amount of data, not the span of time the recording covers.
...
- `files` lists every CSV written with its data row count, again excluding the header row. Add one
  per file if you are checking line counts on disk. `metadata.json` describes the run and is not
...
- `rate_groups` records the grouping decision: for each output file, its sampling rate, the columns
  ...
  machine-readable answer to "which file holds which channel", and it is the field to read if a
...
Diagnostics are worth reading before analysis, not after. `MIXED_SAMPLING_RATES` explains why you
have three signal files. `RECORD_COUNT_MISMATCH` says the recording is shorter than its header
promised. `DEGENERATE_DIGITAL_RANGE` says a channel's calibration is self-contradictory and its
values should not be trusted. `DISCONTINUOUS` says there are gaps in the time column and they are
real.

## Leftovers from an earlier run

`--force` overwrites the files a conversion writes, but it does not empty the directory first.
Converting a mixed-rate recording into a directory and then converting a single-rate one into the
same place leaves `signals_256hz.csv` sitting next to a fresh `signals.csv`, both looking current
and only one of them being so.

`edf2csv` detects this and warns with `STALE_OUTPUT`, naming the files that were not rewritten. It
does not delete anything, because deciding which of two conversions you meant to keep is not the
converter's call. Delete them yourself, or convert into a fresh directory.
