...
EDF's four free-text header fields have had two warnings about where they land for as long as
they have been written into the CSV. `FORMULA_LABEL` for text a spreadsheet runs instead of
reading: a channel labelled `=1+1` opens as a column headed `2`, one labelled
`=HYPERLINK(...)` as a link nobody in the reading chain wrote. `NONPRINTABLE_LABEL` for bytes
that drive a terminal: `\x1b[2J` clears the screen, so `cat signals.csv` can hide the rest of a
session. Neither rewrites anything — the cell says what the recording says — and both exist so
that the tool is not silent about where the text goes. SECURITY.md states the rule: writing
these fields through verbatim is deliberate, "what is not acceptable is the tool being silent
about where that lands."

`annotations.csv`'s `description` column is the same kind of text, out of the same file, into
the same spreadsheet. Nothing was said about it at all.

```text
onset_s,duration_s,description,record_index
0.5,,"=HYPERLINK(""http://example.invalid"",""Sleep stage W"")",0
1.5,,=1+1,1
```

Exit 0, no warning, no mention on any page. Open that in Excel and the description column is a
live link and the number 2. CSV quoting does not help: a spreadsheet strips the quotes and then
reads the cell, which is the whole reason `FORMULA_LABEL` is a warning about a quoted field in
the first place.

It is the likelier of the two to happen. A channel label is written once by the recorder; a
description is typed by a person at a scoring station, or generated by a tool that pasted a
computed name in. And it is the only free text in the output that can carry a character above
U+00FF — header text is decoded latin1, so every byte of it becomes a code point below U+0100 —
which makes it the only field that can hold a bidirectional override. `stage-\u202efdp` is
written faithfully and reaches a terminal as `stage-pdf`.

Both codes are raised for it now, from the conversion and from `--info`:
...
thousands of events and a warning each is not a report. Counted over the rows that reach the
file, after the same `--start` / `--end` filter the writer applies, which is the discipline the
two annotation-duration warnings were given for the same reason. The same two exceptions the
signs take in a header field are taken here — a lone `-`, and a description that is entirely a
number — and the bidi *marks* U+200E and U+200F are not flagged, since they reorder the neutral
characters beside them rather than a run and are how a right-to-left description is written.

**Why this is a minor rather than a patch.** Nothing is rewritten and no output changes, but a
recording that converted with no warnings can now raise one, and `--strict` turns any warning
into exit 1. A pipeline running `edf2csv --strict` over a folder of scored studies can start
failing on files it accepted yesterday. That is the intended effect — the warning is the point —
but it is a change in what a passing run means, and it belongs behind a version number that
says so.

Under the hood, the character class that decides what a terminal will not print as itself now
lives in one place, `src/format/unprintable.ts`, shared by the escaping `--info` does and the
question the conversion asks. It was two copies for one release.
